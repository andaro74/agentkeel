"""The corpus ingest stack, AgentkeelIngest (Security seat; SPEC/03 §6, seed S5's reader's other half).

Deployed by the human with admin, as the bootstrap stack is; no workflow and
no platform role deploys it. SPEC/03 §6 names every part before this file:

    cd infra/ingest && npx aws-cdk@2 diff && npx aws-cdk@2 deploy

- **quarantine**: versioned, private, TLS only. The human uploads here, and
  every new object version invokes the promoter;
- **the promoter**: one Lambda (`promoter.py`, inlined: the account is not
  ...
  and copies the bytes to production under the key `<sha256>` only if
  `data/corpus/admitted.yaml` names that sha256. The list is read here, at
  synth: a change to it reaches the promoter only by the human's deploy;
- **production**: versioned, private, TLS only, Object Lock COMPLIANCE for
  1 day (the human as Security, 2026-09-25). Its policy denies PutObject to
  every principal but the promoter's role, and lets the eval role read;
- **the record**: one item per object version that reached quarantine; its
  resource policy lets the eval role read it.
...
def admitted_sha256s(path: Path = ADMITTED) -> list[str]:
    """The sha256 of every admitted document, sorted. The promoter's whole list."""
    return sorted(entry["sha256"] for entry in yaml.safe_load(path.read_text(encoding="utf-8")))
...
class IngestStack(cdk.Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        eval_role = f"arn:aws:iam::{self.account}:role/{EVAL_ROLE_NAME}"
        ...
        quarantine_name = f"agentkeel-quarantine-{self.account}"
        ...
        production = s3.Bucket(
            self, "Production", bucket_name=f"agentkeel-corpus-{self.account}", versioned=True,
            block_public_access=s3.BlockPublicAccess.BLOCK_ALL, enforce_ssl=True,
            encryption=s3.BucketEncryption.S3_MANAGED, removal_policy=cdk.RemovalPolicy.RETAIN,
        ...
        record = dynamodb.Table(
            self, "Record", table_name="agentkeel-ingest-record",
        ...
        role = iam.Role(
            self, "PromoterRole", role_name="agentkeel-promoter",
            assumed_by=iam.ServicePrincipal("lambda.amazonaws.com"),
            description="agentkeel: the corpus promoter. Promotes only what admitted.yaml names (SPEC/03 §6).",
        )  # fmt: skip
        log_group = logs.LogGroup(self, "PromoterLogs", log_group_name="/agentkeel/ingest/promoter",
        ...
        function = lambda_.Function(
            self, "Promoter", function_name="agentkeel-promoter", role=role,
            ...
            memory_size=256, log_group=log_group, reserved_concurrent_executions=2,
            environment={
                "ADMITTED": json.dumps(admitted_sha256s()), "PRODUCTION": production.bucket_name,
                "RECORD": record.table_name, "GUARDRAIL_ID": pin["id"], "GUARDRAIL_VERSION": pin["version"],
            },
        )  # fmt: skip
        ...
        # Production: only the promoter writes; the eval role reads (observe_ingest.py).
        ...
        production.add_to_resource_policy(iam.PolicyStatement(
            sid="TheEvalRoleReads", principals=[iam.ArnPrincipal(eval_role)],
            actions=["s3:GetObject", "s3:ListBucket"],
            resources=[production.bucket_arn, production.arn_for_objects("*")],
        ))  # fmt: skip
        record.add_to_resource_policy(iam.PolicyStatement(
            sid="TheEvalRoleReadsTheRecord", principals=[iam.ArnPrincipal(eval_role)],
...
stack = IngestStack(
    ...
    description="agentkeel M03 corpus ingest: quarantine, promoter, locked production, record. Security seat.",
...
SUPPRESSIONS = {
    "PromoterRole/DefaultPolicy/Resource": (
        "AwsSolutions-IAM5",
        "SPEC/03 §6, the promoter, which keeps M03's seed S5 (the unsigned amendment) out of production. Three "
        "wildcards, each one bucket's or one guardrail's: GetObject on quarantine/* because any key may be "
        "uploaded; PutObject on production/* because the key is the sha256 of the bytes; ApplyGuardrail on "
        "refagent's guardrail and <arn>:*, its numbered versions. SPEC/01 §6's boundary rule holds: the role "
        "carries the deploy boundary.",
    ),
    "Quarantine/Resource": (
        "AwsSolutions-S1",
        "SPEC/03 §6: M03's seed S5 is read from the promoter's record (one item per object version) and from "
        "the production bucket itself, not from access logs. S3 access logs and data events on the corpus "
        "buckets are M05's, evidence integrity (security-reviewer NOTE 6 at M03 PR 1).",
    ),
    "Production/Resource": (
        "AwsSolutions-S1",
        "SPEC/03 §6: M03's seed S5 is read from the promoter's record and from this bucket by key. Every "
        "object here is locked (COMPLIANCE, 1 day) and only the promoter may put one. S3 access logs and data "
        "events are M05's (security-reviewer NOTE 6 at M03 PR 1).",
    ),
}
