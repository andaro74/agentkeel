...
## Security (Finding S-1, what PR 2 left, and what the close's Security read added)

| # | Item | Seat | When |
|---|---|---|---|
| 12 | S-1's last step: a run of `evals.yml` that assumes the **redeployed** role, showing STS evaluates the `job_workflow_ref` condition. The stack was redeployed 2026-09-19T00:20:25Z; the M00 close PR's own run is the first to assume it and is recorded in `milestones/M00/rulings/pr3.md`. Security signs S-1 on that observation, and corrects `infra/eval-role/README.md`'s "Not observed yet" bullet in the same PR — it is a measured path, so the edit rides with M01's first measurement. | Security | before M01 PR 1 |
...
| 17 | The five pinned action SHAs were resolved from each repo's latest release through the API, not checked with `git ls-remote`. A 40-hex pin is immutable whatever the trailing `# v4.2.2` comment says, so what is unverified is only that each SHA exists upstream and is the tag it claims — and a wrong SHA fails the checkout immediately. Check them once; the risk is lower than PR 2 recorded. | Security | before M01 PR 1 |
...
| 28 | No permission boundary on `agentkeel-m00-evals`. R3 says the PoC runs on accounts with boundaries. The boundary is M01's bootstrap stack; item 20 carries only the name collision, not this. Raised by the Security read of the M00 close. | Security | at M01 PR 1 |
| 29 | Anyone with write access can edit `.github/workflows/evals.yml`, keep the file name so `job_workflow_ref` still matches, and change what is measured and what is spent. Nothing gates that until M02. SPEC/00 §5 schedules `workflow file hash unchanged` in `validate` from M01 PR 1; it is not in any carried list until now. | Security | at M01 PR 1 |
| 30 | ADR-0004 amendment 1's merge-commit rule is a convention: the ruleset on `main` still allows `squash` and `rebase`. Removing both from `allowed_merge_methods` makes it mechanical and costs nothing. Security rules the ruleset, Product the ADR text. | Security, with Product | before M01 PR 1 |
| 31 | The repository ruleset is Security's by assertion and lives in no file, so no gate reads it. It decides whether the merge rule holds (item 30) and whether the owner can bypass (item 15), and SPEC/00 §5 does not list it among Security's paths while §5 says every file on `main` has a seat. Export it to a Security-owned file that `validate` diffs, or have SPEC name it as out-of-tree and say which milestone brings it in. | Security, with Product | at M01 open |
| 32 | `infra/eval-role/README.md` records zero cdk-nag findings and zero suppressions, and nothing in the tree can check it: `cdk.out/` is gitignored and no `make` target synths. SPEC/00 §5 puts cdk-nag in `validate` from M01 PR 1. Commit the report under a Security path, or date the claim to when `validate` runs it. | Security | at M01 PR 1 |
| 33 | The role has no explicit `Deny`. "What it cannot do: anything else" is true of the inline policy as written, and with no boundary (item 28) nothing stops a later attach. One `Deny` statement on `iam:*`, `sts:AssumeRole`, `logs:Delete*` and the guardrail actions costs one statement. | Security | at M01 PR 1 |
| 34 | The worst case for spend has no number. The README gives the expected cost (15 calls at 512 output tokens) and then says "That is not the worst case" — a PR step can mint its own token, hold the role for an hour and invoke at the account's quota. Write the figure, or land the Budgets alarm of item 13 first. | Security, with Threshold Owner | at M01 open |
| 35 | The trust policy's `StringLike` pattern `…@refs/pull/*/merge` is described as "the `*` is the PR number"; IAM's `*` also matches `/`. Not exploitable — GitHub issues the ref, not the caller — but the comment claims more than the pattern says. Narrow it or correct the comment. | Security | at M01 PR 1 |
