...
| 24 | **A customer key for the rights table is a change to R4's deny, not a construct edit.** Added at M02 PR 1 from the `security-reviewer` report (§3): a customer key renders `SSEEnabled: true` too, so the DynamoDB handler calls `kms:CreateGrant`, which `agentkeel-deploy-boundary` denies on `*` and the key policy denies to `agentkeel-cfn-exec` and `agentkeel-deploy` by name. M05's key needs a `kms:GrantIsForAWSResource` condition on the deny, or a per-key exception in the key policy, ruled by Security, and `tests/test_bootstrap.py`'s `NEEDED_BY_PROPERTY` will find the collision at synth in M05 PR 1 rather than at CREATE a second time. | Security | M05 open |
